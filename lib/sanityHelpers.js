...
export async function getIndexPage() {
  try {
    ...
    const worksQuery = `*[_type=='works' && !(_id in path('drafts.**')) ]| order(_createdAt desc){Categories[]->,_createdAt,_id,description,githubUrl,liveUrl,mainImage,photoGallery,title,youtubeUrl}`;
...
export async function getBlogBySlug(slug) {
  try {
    const blogQuery = `*[_type=='blogPost' && slug.current == '${slug}' && !(_id in path('drafts.**')) ][0]{comments,Categories[]->,Content,_createdAt,_id,description,_updatedAt,slug,title,image}`;
...
export async function getBlogPreviewBySlug(slug) {
  try {
    const blogQuery = `*[_type=='blogPost' && slug.current == '${slug}'  ][0]{comments,Categories[]->,Content,_createdAt,_id,description,_updatedAt,slug,title,image}`;
    let blog = await sanityClient.fetch(blogQuery, {});
