import { getCategories, getIndexPage } from '../../../lib/sanityHelpers';
  ...
  return (
    <>
      <NextSeo
        ...
        canonical={`https://taoufiqlotfi.tech/blog/category/${slug}`}
        openGraph={{
          title: seoTitle,
          description: seoDescription,
          type: 'website',
          locale: 'en',
          url: `https://taoufiqlotfi.tech/blog/category/${slug}`,
          images: [
            {
              url: seoImage,
              width: 1912,
      ...
      <div className="realative  w-full h-64 breadcrumb-bg">
        ...
        <div className="relative flex flex-col justify-center items-center h-full w-full">
          ...
          <h1 className="capitalize flex mb-2 text-secondary-400 antialiased font-bold text-4xl	pt-20">
            Blogs :{' '}
            <span className="ml-2  hover:cursor-pointer flex flex-1 justify-between items-center hover:text-secondary-400 text-[#bfbecb] mt-px ">
...
export async function getStaticProps(context) {
  ...
  blogs.forEach((blog) => {
    ...
    blogCategories.forEach((category) => {
      let cslug = category?.slug?.current;
      if (slug == cslug) {
        currentBlogs.push(blog);
        currentCategory = category;
      }
      if (!categoriesCount.hasOwnProperty(cslug)) {
        categoriesCount[cslug] = 1;
        categories.push(category);
      } else {
        categoriesCount[cslug] = categoriesCount[cslug] + 1;
      }
    });
